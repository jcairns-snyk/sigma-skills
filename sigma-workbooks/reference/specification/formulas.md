...
> **`CountNonNull()` is not a real Sigma function**, despite reading like the obvious name for "count the non-null rows." It compiles without complaint at spec-authoring time, then fails at **query time** with `Reference to errored column` — a failure that only surfaces when the element is actually queried or rendered, not at POST or `/verify`. `Count([col])` already skips null values, so it's a direct drop-in replacement; there's no missing capability here, just a function name Sigma doesn't have.
...
## Design pattern: derive counts and labels, don't hand-type them

A user-facing string like "8 of 19 metrics live" or "13 held" is tempting to type directly into a `text` element or a KPI's `name` — but it drifts the moment the underlying list changes, and nothing revalidates a hand-typed number against reality. It's exactly the kind of fact that looks like live data but silently goes stale the next time a metric is added or removed. Prefer computing any such count from the data itself — `Count()` / `CountDistinct()` over the real rows in a table/KPI bound to the source of truth — so the number and the thing it's counting can never disagree. If the workbook is generated by a script, the same idea applies one level up: derive the count from `len()` on the same list that generates the tiles/rows, rather than typing the number into the template separately.

This generalizes the "one source of truth" pattern in `controls.md`'s picker/branch-formula guidance: wherever two things in a spec need to stay in agreement — a count and a list, a picker's options and a formula's branches, a label and the data behind it — generate one from the other (or both from a shared source) instead of hand-typing both.
